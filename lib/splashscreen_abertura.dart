...
class _MySplashScreenState extends State<MySplashScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _controller = AnimationController(
      vsync: this,
      duration: Duration(seconds: 3), // Ajuste a duração total da animação
    );

    _letterAnimations = List.generate(
      13, // Número de letras no texto 'Carregando...'
    ...
    _offsets = List.generate(13, // Número de letras no texto 'Carregando...'
      (index) => 0.0, // Valor inicial dos deslocamentos das letras
    );
  ...
  double _waveOffset(double controllerValue, int index) {
    const double waveFrequency = 6.0; // Ajuste a frequência da onda
    const double waveAmplitude = 1.5; // Ajuste a amplitude da onda
    const double speedScale = 1.6; // Ajuste a escala de velocidade

    // Calcula um atraso baseado na posição da letra e no valor do controlador
    double waveDelay =
        math.sin(controllerValue * 2 * math.pi * waveFrequency + index) * waveAmplitude * speedScale;
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              Text("Sistemas de Vendas", style: TextStyle(
                  fontSize: 15,
                  fontWeight: FontWeight.bold,
                  fontFamily: "TAN Nimbus",
                  color: Color(0xFFE9F1F1),
